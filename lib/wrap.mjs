...
export const PACKAGE = "@darkvector/witness";
...
export function installedFromRegistry(bin = BIN) {
  ...
  return parts.includes("_npx") || (parts.includes("node_modules") && parts.includes("@darkvector"));
}
...
/**
 * Wrap one config entry. `via: "path"` runs `node <bin>` (a checkout); `via: "npx"` runs `npx -y @darkvector/witness@<version>`
